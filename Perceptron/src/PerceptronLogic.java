...
public class PerceptronLogic {
	 ...
	 * 
	 * Method to Train a Perceptron. Prints out iterations done and error when stop
	 * condition is met return an object of PerceptronData-type
	 * 
	 */
	public static PerceptronData trainPerceptronSequentially(ArrayList<Point> points, int maxIterations) {
		...
		do {
			...
			for (Point point : points) {
				...
				if (localError != 0)
					misclassifiedPoints.add(point);
			}

			// save if better than weightset in pocket
			if (globalError < bestSeparator.getError())
				bestSeparator = new PerceptronData(weights, globalError);
	...
	public static PerceptronData trainPerceptronParallel(ArrayList<Point> points, int maxIterations) {
		...
		// TODO vvvvvvvvvvvvvvvvvvvvvvvvvvvvvv adjust to work in parallel
